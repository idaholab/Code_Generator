...
/**
 * \file udp-cosem.h
 * \brief The udp cosem class subclass of Application, modeled on udp.
...
/**
 * \ingroup generator
 * \brief The UDP cosem subclass from Application.
 *
 *  The udp-cosem class is a subclass of application.
 *
 *  This class represent a udp cosem application.
 *
 *  It re-writte some methods which are called to generate ns3 code.
 *
 *  Usage:
 *    - Create an application with udp-cosem type.
 */
class UdpCosem : public Application
{
  private:
    /**
  ...
  public:
     ...
     * \param port port
     */
    UdpCosem(const std::string &type, const size_t &indice, const std::string &senderNode, const std::string &receiverNode, const size_t &startTime, const size_t &endTime, const size_t &port);
