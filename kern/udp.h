...
/**
 * \file udp.h
 * \brief The udp class subclass of Application, modeled on udp echo.
...
/**
 * \ingroup generator
 * \brief The UDP subclass from Application.
 *
 *  The udp class is a subclass of application.
 *
 *  This class represent a udp application.
 *
 *  It re-writte some methods which are called to generate ns3 code.
 *
 *  Usage:
 *    - Create an application with udp type.
 */
class Udp : public Application
{
  private:
    /**
  ...
  public:
     ...
     * \param port port
     */
    Udp(const std::string &type, const size_t &indice, const std::string &senderNode, const std::string &receiverNode, const size_t &startTime, const size_t &endTime, const size_t &port);
